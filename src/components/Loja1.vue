...
const fetchProdutos = async () => {
  try {
    const response = await fetch('http://127.0.0.1:5000/products?db=mongodb');
    if (!response.ok) {
...
const realizarVenda = async () => {
  ...
  try {
    const response = await fetch(`http://127.0.0.1:5000/product/${produtoSelecionado.value.id_produto}?db=mongodb`, {
      method: 'PUT',
      headers: {
